...
Live `brain_service_status` still reports
`build_info.fastmcp_cloud_git_commit_sha = 836baf49…` and `version = 1.17.6`
after main moved through the #236 bump (`562f5ee` → 1.17.7) and the later
`84f11f36` merge. No code defect — deploy smoke passes and `__version__`
matches pyproject. The 1.17.7 metadata bump never made it onto Horizon, so
bump again to 1.17.8 and refresh `.deploy-trigger` so Horizon must rebuild
from HEAD.
...
Picks up the SDK's fix for GHSA-m2h6-j472-rp4c: the X.509 verifier accepted wildcard DNS SANs, escaping `permittedSubtrees`. The advisory is fixed in cryptography 49.0.0, and the SDK previously declared a floor of `>=46.0.5` — which admitted every affected release.

No install here was exposed: the resolved lock already carried a patched cryptography. What changes is what a fresh resolve is *allowed* to land on. See tollbooth-dpyc v0.88.1.
...
### Added
- `update_thought` now accepts `new_parent_id`, so a single call can set any subset of {name, label, type, parent} on one thought. Reparenting reuses the morpher's link-surgery (replaces the existing parent link, does not add one), removing the two-call `morph_thought` + `update_thought` dance for routine restructures like Kanban column moves. Still one billable unit — single-thought scope. (Brain task 06e4610c, #163)

### Changed
- Internal: extracted the reparent link-surgery from `morph_thought` into a shared `reparent_thought` helper consumed by both tools (DRY); `morph_thought` behavior is unchanged.

### Documentation
- `morph_thought` and `update_thought` tool descriptions now state reparent semantics explicitly: reparenting *replaces* the existing parent child-link (old deleted, new created); it does not add an additional parent. (Brain task 27deaf6f, #163)
